...
def sar_model_evaluation(descriptors: pd.DataFrame):
    ...
    model1, y_pred1, metrics1 = fit_RandomForestRegressor(X_train, X_test, y_train, y_test) # hyperparameters tuning with selected function
...
def sar_model_train(descriptors_train: pd.DataFrame, indices): 
    ...
    b = StandardScaler() 
    clf = RandomForestRegressor() # selected model from sklearn
...
if __name__ == "__main__":
    ...
    pred_data = pd.read_csv("logp_100.csv")
